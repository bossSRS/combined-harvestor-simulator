...
namespace ChobiAssets.PTM
{
    public class Drive_Control_CS : MonoBehaviour
    {
		...
		*/
        ...
        public float Max_Speed = 1.6f;
        public float Min_MaxSpeed = 1.6f;
        public float Max_MaxSpeed = 9.5f;
        ...
        private CHS_Controller chs_Controller;
        ...
        void Initialize()
        {
            ...
            chs_Controller = GetComponentInParent<CHS_Controller>();
        }
        ...
        {
            Max_Speed = Mathf.Abs(Value);
            Max_Speed = Mathf.Clamp(Max_Speed, Min_MaxSpeed, Max_MaxSpeed);
        }
        ...
        void FixedUpdate()
        {
            ...
            Current_Velocity = thisRigidbody.linearVelocity.magnitude;
        ...
        void Set_Driving_Values()
        {
            ...
            float currentTorqueRPM = (RPM > 0) ? RPM : defaultTorque;
            if (chs_Controller != null && chs_Controller.harvestorState == HarvestorState.Off)
            {
                currentTorqueRPM = 0f;
            }
            else if (currentTorqueRPM < 500f)
            {
                currentTorqueRPM = 1200f;
            }
            Left_Torque = currentTorqueRPM * -Mathf.Sign(leftSpeedRate) * Mathf.Ceil(Mathf.Abs(leftSpeedRate)); // (Note.) When the "leftSpeedRate" is zero, the torque will be set to zero.
            Right_Torque = currentTorqueRPM * Mathf.Sign(rightSpeedRate) * Mathf.Ceil(Mathf.Abs(rightSpeedRate));
        }
            ...
            { // Not stopping, Not pivot-turning, Not brake-turning.
                ...
                { // The difference is large.
                    ...
                    { // Almost horizontal.
                        ...
                        {
                            leftSpeedRate = Mathf.MoveTowards(leftSpeedRate, currentActualSpeedRate * Mathf.Sign(leftSpeedRate), 5000.0f * Time.deltaTime);
                        }
                        if (rightSpeedRate != 0.0f)
                        {
                            rightSpeedRate = Mathf.MoveTowards(rightSpeedRate, currentActualSpeedRate * Mathf.Sign(rightSpeedRate), 5000.0f * Time.deltaTime);
                        }
        ...
        void Anti_Slip()
        {
            ...
            { // The ray hits the ground.
                ...
                Vector3 tempVelocity = thisRigidbody.linearVelocity;
                if (Speed_Rate == 0.0f)
                { // The tank stops.
                ...
                { // The tank should been driving.
                    ...
                    var sign = Mathf.Sign(thisTransform.InverseTransformDirection(thisRigidbody.linearVelocity).z);
                ...
                thisRigidbody.linearVelocity = tempVelocity;
            }
        }
        ...
        void Limit_Torque()
        {
            ...
            { // The tank should be going forward.
                RPM = Mathf.Lerp(defaultTorque, 0.0f, torqueRate);
            }
            else
