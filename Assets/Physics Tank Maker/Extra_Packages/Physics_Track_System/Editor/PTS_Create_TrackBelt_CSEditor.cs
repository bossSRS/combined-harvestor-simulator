	...
	public class PTS_Create_TrackBelt_CSEditor : Editor
	{
        ...
        void Set_Inspector ()
		{
			...
			if (EditorApplication.isPlaying == false) {
				...
				Collider_MaterialProp.objectReferenceValue = EditorGUILayout.ObjectField ("Physic Material", Collider_MaterialProp.objectReferenceValue, typeof(PhysicsMaterial), false);
		...
		void Create_TrackPiece (string direction, Vector3 position, float angleY, int number)
		{
			...
			if (Use_BoxColliderProp.boolValue) {
				...
				boxCollider.material = Collider_MaterialProp.objectReferenceValue as PhysicsMaterial;
			} else {
				// CapsuleColliders
				...
				horizontalCollider.material = Collider_MaterialProp.objectReferenceValue as PhysicsMaterial;
				...
				verticalCollider.material = Collider_MaterialProp.objectReferenceValue as PhysicsMaterial;
			}
			// Stabilizer_CS
		...
		void Finishing (string direction)
		{
			...
			for (int i = 1; i <= thisTransform.childCount; i++) {
				...
				if (basePiece) {
					...
					rigidbody.angularDamping = Angular_DragProp.floatValue;
					// for Static_Track creating
					if (Static_FlagProp.boolValue) {
						rigidbody.linearDamping = 10.0f;
					}
				}
			}
			// Add HingeJoint.
